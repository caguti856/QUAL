...
from transformers import pipeline  # <-- added for zero-shot AI detection
...
# ==============================
# AI DETECTION (rules + transformers)
# ==============================
...
# Symbols that look like templated / markdown / “AI prompt” formatting, including __
SYMBOL_RX = re.compile(
    r"(?:__|\*\*|`{2,}|==>|=>|->|<-|<->|"      # markdown / code / arrow chains
    r"[≥≤≧≦≈±×÷%]|"                            # math / percentages
    r"[→←⇒↔↑↓]|"                               # arrows
...
def _rule_ai_score(text: str, question_hint: str = "") -> float:
    """
    Heuristic AI suspicion score 0..1.

    Design:
    - Long, rich sentences are NOT penalised.
    - Strongest signals: explicit "as an AI", heavy symbol / formatting cues, strict timeboxing.
    - Buzzwords and transitions are only soft hints.
    """
    t = clean(text)
    if not t:
        return 0.0
    ...
    # 1) Hard cues – very strong signals
    if AI_RX.search(t):
        score += 0.50   # self-declared AI is a big red flag

    # Count 'fancy' symbols / formatting (__, **, arrows, math etc.)
    symbol_hits = len(SYMBOL_RX.findall(t))
    if symbol_hits >= 3:
        score += 0.30
    elif symbol_hits == 2:
        score += 0.20
    ...
    # Time-boxed planning language ("day 1", "week 2", "in 3 weeks")
    if TIMEBOX_RX.search(t):
        score += 0.15

    # 2) Structural cues – small nudges only
    ...
    # 3) Buzzwords – only if several appear
    buzz_hits = sum(1 for b in AI_BUZZWORDS if b in t.lower())
    if buzz_hits >= 3:
        score += 0.10
    elif buzz_hits == 2:
        score += 0.05
    # 0 or 1 buzzword = ignore (humans also use them)

    # 4) Q/A overlap – generic answer relative to question
    if question_hint:
...
def get_ai_zero_shot_classifier():
    """
    Zero-shot classifier that estimates how much a response looks 'AI-written'.
    Uses a generic NLI model; this is indicative, not proof.
    """
    return pipeline(
...
def _model_ai_score(text: str) -> float:
    """
    Return a probability 0..1 that the text is AI-written, based on zero-shot.
    Very short answers are treated as low-confidence.
    """
    t = clean(text)
    if not t:
        return 0.0

    # Skip very short answers – model not reliable there
    if len(t.split()) < 8:
        return 0.0

    clf = get_ai_zero_shot_classifier()
    labels = ["AI-written answer", "human-written answer"]

    try:
        res = clf(t, labels)
    except Exception:
        # If something goes wrong (no internet, model issue), don't crash the app
        return 0.0

    scores = dict(zip(res["labels"], res["scores"]))
    return float(scores.get("AI-written answer", 0.0))


def ai_signal_score(text: str, question_hint: str = "") -> float:
    """
    Final AI suspicion score 0..1 combining:
      - rule-based patterns (primary for your symbols, '__', '=>', etc.)
      - zero-shot transformer model (secondary confirmation)

    We only call the transformer when the rule score is already non-trivial,
    to save compute and avoid over-flagging.
    """
    rule_s = _rule_ai_score(text, question_hint)

    # If the rules see almost nothing, don't bother with the heavy model.
    if rule_s < 0.20:
        return rule_s

    model_s = _model_ai_score(text)

    # Combine: rules still matter most, model is a second opinion.
    if model_s == 0.0:
        combined = rule_s
    else:
        combined = 0.6 * rule_s + 0.4 * model_s
