...
# Map human-readable attributes -> KoBo root folder names seen in exports
ATTR_TO_ROOT = {
    "Strategic Positioning & Donor Fluency": "Strategic_Positioning",
    "Power-Aware Stakeholder Mapping": "Power_Aware",
    "Equitable Allyship & Local Fronting": "Equitable_Allyship",
    "Coalition Governance & Convening": "Coalition",
    "Community-Centered Messaging": "Community_Centered_Messaging",
    "Evidence-Led Learning (Local Knowledge)": "Evidence_Led",
    "Influence Without Authority": "Influence_Without_Authority",
    "Risk Management & Adaptive Communication": "Risk_Management",
}
...
def build_bases_from_qid(question_id: str, attribute: str = "") -> list[str]:
    """Return plausible base prefixes for a given qid using the correct KoBo roots.
    Supports both variants with and without "/A#_Section/" in the path.
    """
    out: list[str] = []
    if not question_id:
        return out
    ...
    # Prefer the exact root observed for this attribute, with safe fallbacks
    roots = []
    root_from_attr = ATTR_TO_ROOT.get(attribute or "")
    if root_from_attr:
        roots.append(root_from_attr)
    roots += ["Networking", "networking"]

    for r in roots:
        out.append(f"{r}/{sect}_{qn}")            # variant without Section folder
        out.append(f"{r}/{sect}_Section/{sect}_{qn}")  # legacy variant with Section folder
    return out
...
def resolve_kobo_column_for_mapping(df_cols: list[str], question_id: str, prompt_hint: str, attribute: str = "") -> str | None:
    ...
    # 1) Direct base candidates using attribute-aware roots + both path variants
    ...
    # 2) Token fallback on A#_n (e.g., A1_3) anywhere in the header
    token = None
...
def score_dataframe(df: pd.DataFrame, mapping: pd.DataFrame,
    ...
    for r in all_mapping:
        ...
        if hit:
            resolved_for_qid[r["question_id"]] = hit
        else:
            missing.append((r["question_id"], r["attribute"], r.get("prompt_hint","")))

    with st.expander("🧭 Mapping → Kobo column resolution (attribute-aware)", expanded=False):
        if resolved_for_qid:
            st.dataframe(pd.DataFrame(list(resolved_for_qid.items()), columns=["question_id","kobo_column"]))
        if missing:
            st.warning(f"{len(missing)} mapping rows not found in headers (showing up to 24).")
            st.dataframe(pd.DataFrame(missing[:24], columns=["question_id","attribute","prompt_hint"]))
