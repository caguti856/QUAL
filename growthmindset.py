...
def fetch_kobo_dataframe() -> pd.DataFrame:
    ...
    for kind in ("submissions","data"):
        ...
        try:
            ...
            return pd.DataFrame()
    st.error("Could not fetch data. Check KOBO_BASE, KOBO_ASSET_ID2, token permissions.")
    return pd.DataFrame()
...
def build_kobo_bases_from_qid(question_id: str) -> list[str]:
    ...
    roots = [
        "Growthmindset",      # no space
        "Growth Mindset",     # with space
    ]
...
def _score_kobo_header(col: str, token: str) -> int:
    ...
    if "growthmindset/" in c or "growth mindset/" in c or "/a" in c: score += 2
    return score
...
def score_dataframe(df: pd.DataFrame, mapping: pd.DataFrame,
    ...
    with st.expander("🔎 Debug: Growthmindset section columns present", expanded=False):
        sample_cols = [c for c in df_cols if "/A" in c or "Growthmindset/" in c or c.startswith("A")]
...
# ==============================
def main():
    st.title("📊 Growth Mindset: Kobo → Scored Excel / Google Sheets")
