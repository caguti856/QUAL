...
# answers that should be treated as "no response" and forced to 0
_NULLISH_ANSWERS = {
    "", "none", "null", "n/a", "na", "no response", "no answer",
    "blank", "nil", "nill", ".", "-", "--", "_", "n\\a"
}


def is_nullish_answer(ans: str) -> bool:
    """
    Treat blank / 'none' / 'null' / 'n/a' / tiny one-word fillers as no response.
    These will be scored as 0 and skipped for embeddings + AI detection.
    """
    t = clean(ans).lower()
    if t in _NULLISH_ANSWERS:
        return True

    tokens = t.split()
    if len(tokens) == 1 and len(t) <= 5:
        # e.g. "ok", "yes", "no", "good", "fine"
        return True
...
def score_dataframe(df: pd.DataFrame, mapping: pd.DataFrame,
    ...
    # batch-embed all distinct non-nullish answers once
    ...
    for _, row in df.iterrows():
        for r in all_mapping:
            ...
            if col and col in df.columns:
                a = clean(row.get(col, ""))
                if a and not is_nullish_answer(a):
