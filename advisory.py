...
def score_dataframe(df: pd.DataFrame, mapping: pd.DataFrame, packs_by_qid: dict[str, ExemplarPack]) -> pd.DataFrame:
    ...
    with st.expander("🔎 Advisory-like columns found", expanded=False):
        sample_cols = [c for c in df_cols if "/A" in c or "/B" in c or "/C" in c or "/D" in c or "/E" in c or "/F" in c or "/G" in c or "/H" in c or "Advisory/" in c]
