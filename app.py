...
MAP_PATH     = DATASETS_DIR / "mapping.csv"  # make sure this exists
...
def build_resolution_map(df: pd.DataFrame, mapping: pd.DataFrame) -> tuple[dict, list]:
    ...
    def resolve_df_column(wanted: str) -> str | None:
        if wanted in df.columns:  # exact
            return wanted
        w_norm = normalize_col_name(wanted)
        if w_norm in norm_to_dfcols:  # normalized exact
...
def score_dataframe(df: pd.DataFrame, mapping: pd.DataFrame,
    ...
    with st.expander("🔎 Column resolution details"):
        ...
        if unresolved:
            st.warning("Unresolved mapping columns (fix these in DATASETS/mapping.csv):")
    ...
    for i, resp in df.iterrows():
        ...
        for r in all_mapping:
            ...
            # Resolve question key: question_id first, else fuzzy on prompt_hint
            qkey = (qid or "").strip()
            ...
            if qkey and qkey in q_centroids:
                ...
                # optional QA-overlap clamp (same as your Colab)
                qtext = by_qkey[qkey]["question_text"]
                if qa_overlap(ans, qtext or qhint) < MIN_QA_OVERLAP and sc is not None:
                    sc = min(sc, 1)  # clamp to at most Compliant

            # 2) attribute-level fallback
            if sc is None and attr in attr_centroids:
                sc = cos_best(vec, attr_centroids[attr])
                if qa_overlap(ans, qhint) < MIN_QA_OVERLAP and sc is not None:
                    sc = min(sc, 1)
