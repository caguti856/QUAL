# dashboard.py
# ------------------------------------------------------------
# FULL VERSION (keeps your colors + keeps your previous visuals)
# ✅ Keeps ALL previous section visuals:
#    - Avg = chart ; Rank = table
#    - Per question: Score chart + score table ; Rubric chart + full table in expander
#    - Heatmap shown once per section
# ✅ Removes the old staff multiselect filter (no filtering the whole dataset)
# ✅ Adds TWO NEW tabs:
#    1) Staff Summary  -> one row per staff (overall + section totals)
#    2) Individual Profile -> pick ONE staff and see their section totals + question heatmap
# ✅ Each new tab has its own filter/select (scoped ONLY to that tab)
...
def first_existing(df: pd.DataFrame, candidates: List[Optional[str]]) -> Optional[str]:
    for c in candidates:
...
# ==============================
def staff_id_column(df: pd.DataFrame) -> Optional[str]:
    candidates = [
        "Care_Staff",
        "CARE Staff id",
        "CARE Staff ID",
...
def score_dist(df: pd.DataFrame, col: str) -> pd.DataFrame:
    if not has(df, col):
        return pd.DataFrame({"Score": [0, 1, 2, 3], "Count": [0, 0, 0, 0], "Percent": [0, 0, 0, 0]})
...
# ==============================
# NEW: STAFF SUMMARY + INDIVIDUAL TOTALS
# ==============================
...
def build_staff_summary(df: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    # section total 0–12 derived from Qn columns
    ...
    g = tmp.groupby(tmp[sid].astype(str), dropna=True)

    out = pd.DataFrame(index=g.size().index)
    out.index.name = sid
    out["Rows"] = g.size().values

    if overall_total_col in tmp.columns:
        out["Overall Total (mean)"] = g[overall_total_col].apply(lambda s: pd.to_numeric(s, errors="coerce").mean())
        out["Overall Total (sum)"]  = g[overall_total_col].apply(lambda s: pd.to_numeric(s, errors="coerce").sum())

    for section in cfg["sections"]:
        col = f"{section}__Total(0-12)"
        if col in tmp.columns:
            out[f"{section} Total (0–12)"] = g[col].mean()

    if overall_rank_col in tmp.columns:
        def mode_or_dash(s):
            s2 = _clean_series(s)
            return "-" if len(s2) == 0 else str(s2.value_counts().index[0])
        out["Overall Rank (mode)"] = g[overall_rank_col].apply(mode_or_dash)
...
def render_page(page_name: str):
    ...
    # ==========================
    # SECTION TABS (your original visuals)
    # ==========================
    ...
    # ==========================
    # OVERALL ANALYSIS TAB (your original)
    # ==========================
    with tabs[-3]:
        ...
        ai_col = first_existing(df, [cfg.get("ai_col"), "AI_Suspected", "AI-Suspected", "AI Suspected"])
    ...
    with tabs[-2]:
        ...
        sid = staff_id_column(df)
        if not sid:
            st.error("No Staff ID column detected on this sheet.")
            st.write("Columns found:", list(df.columns))
        else:
            staff_summary = build_staff_summary(df, cfg)
            if staff_summary.empty:
                st.error("Could not build staff summary (missing staff IDs or question columns).")
            else:
                st.caption(f"Detected Staff column: **{sid}** | Staff count: **{staff_summary[sid].nunique()}**")

                search = st.text_input("Filter staff (table only)", "")
    ...
    with tabs[-1]:
        ...
        if not sid:
            ...
            if not staff_ids:
                st.error("Staff ID column exists but has no values.")
            else:
                ...
                if has(df_staff, overall_total_col):
                ...
                # Section totals (0–12) for this staff (derived from Qn1..Qn4)
                sec_rows = []
                for section in cfg["sections"]:
                ...
                if sec_rows:
                    ...
                    fig.update_layout(xaxis_title="", yaxis_title="Section Total (0–12)")
                    safe_plot(fig, key=f"{page_name}-profile-section-totals")

                    with st.expander("Show section totals table"):
                        st.dataframe(sec_df, use_container_width=True, hide_index=True)
                else:
                    st.warning("No section totals available (missing Section_Qn1..Qn4 columns).")

                st.divider()

                # Question heatmap for this staff (mean per question)
                q_rows = []
                for section in cfg["sections"]:
                    for qn in range(1, 5):
                        col = f"{section}_Qn{qn}"
                        if has(df_staff, col):
                            vals = pd.to_numeric(df_staff[col], errors="coerce").dropna()
                            if len(vals):
                                q_rows.append({"Section": section, "Question": f"Qn{qn}", "Score": float(vals.mean())})
                ...
                if q_rows:
                    ...
                    with st.expander("Show question table"):
                        st.dataframe(qdf, use_container_width=True, hide_index=True)
                else:
                    st.info("No question scores found for this staff on this sheet.")
